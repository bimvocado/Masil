...
/>)}
  ...
  pagingEnabled
      ...
      {/* 댓글 모달 & 카테고리 모달 (기존 스타일 및 구조 유지) */}
      <Modal visible={showComments} transparent animationType="slide">
        <TouchableOpacity style={styles.modalBackdrop} activeOpacity={1} onPress={() => { setShowComments(false); setEditingCommentId(null); }} />
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.panel}>
          <View style={styles.panelHeader}>
            ...
            <TouchableOpacity onPress={() => setShowComments(false)}><Text style={{fontSize: 20}}>✕</Text></TouchableOpacity>
          </View>
          <FlatList
            data={comments}
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                      ...
                      {user?.userId === item.userId && (
                        <View style={{ flexDirection: 'row' }}>
                          <TouchableOpacity onPress={() => handleStartEdit(item)}><Text style={styles.editBtn}>수정</Text></TouchableOpacity>
                          <TouchableOpacity onPress={() => handleDeleteComment(item.commentId)}><Text style={styles.deleteBtn}>삭제</Text></TouchableOpacity>
                        </View>
                      )}
                    </View>
          ...
          <View style={styles.inputRow}>
            <TextInput style={styles.input} value={newCommentText} onChangeText={setNewCommentText} placeholder="댓글 입력..." />
      ...
      <Modal visible={showCategoryPicker} transparent animationType="slide">
        ...
        <View style={styles.panel}>
          <ScrollView>
            {categories.map((cat) => (
              <TouchableOpacity key={cat.categoryId} style={styles.categoryItem} onPress={() => handleSelectCategory(cat.categoryId)}>
...
const styles = StyleSheet.create({
  ...
  profilePlaceholder: { width: 30, height: 30, borderRadius: 15, backgroundColor: '#eee', marginRight: 10 },
